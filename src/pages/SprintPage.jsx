            ...
            return (
              <div key={week} className="bg-white dark:bg-slate-800 border border-gray-200 dark:border-slate-700 rounded-xl overflow-hidden">
                <div className="flex items-center gap-2 px-3 py-2 border-b border-gray-100 dark:border-slate-700 bg-gray-50 dark:bg-slate-900/50">
                  ...
                  <span className={`text-xs ${week <= 4 ? 'text-gray-400' : 'text-amber-500'}`}>{week <= 4 ? 'Month 1' : 'Month 2'}</span>
                  <span className="ml-auto text-xs text-gray-400">{weekIdeas.length} idea{weekIdeas.length !== 1 ? 's' : ''}</span>
                </div>
                <div className="p-2 min-h-[44px]">
                  {weekIdeas.length === 0 && (
                    <div className="text-xs text-gray-300 dark:text-slate-600 py-2 px-1 italic">Empty</div>
                  )}
                  {weekIdeas.map(idea => (
                    <div key={idea.id} className="flex items-start gap-2 p-2 mb-1 bg-violet-50 dark:bg-violet-900/20 border border-violet-200 dark:border-violet-800 rounded-lg">
            ...
            <div key={idea.id}>
              <div
                className={`bg-white dark:bg-slate-800 border rounded-xl p-3 cursor-pointer transition-colors
                  ${selected === idea.id ? 'border-violet-400 dark:border-violet-500 bg-violet-50 dark:bg-violet-900/20' : 'border-gray-200 dark:border-slate-700 hover:border-violet-300'}`}
                onClick={() => setSelected(selected === idea.id ? null : idea.id)}
              >
                <div className="flex items-start gap-2">
                  <div className="flex-1 min-w-0">
                    <div className="text-sm font-semibold text-gray-800 dark:text-slate-200 leading-snug">#{idea.id} {idea.name}</div>
